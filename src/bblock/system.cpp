...
//YH
void System::SetPolaris(std::vector<double> polaris) {
    // Make sure that the xyz of input has the right size 
    if (polaris.size() != numat_) {
        std::string text = 
            "Sizes " + std::to_string(polaris.size()) + " and " + std::to_string(numat_ ) + " don't match.";
        throw CUException(__func__, __FILE__, __LINE__, text);
    }

    // Copy each coordinate in the appropriate place in the internal
    // xyz vector 

    for (size_t i = 0; i < nat_.size(); i++) {
        size_t ini = initial_order_realSites_[i].second;
        size_t fin = ini + nat_[i];
        size_t ini_new = first_index_[i];
        std::copy(polaris.begin() + ini, polaris.begin() + fin, polaris_.begin() + ini_new);
    }
}
//EYH
//SY
void System::AddMonomer(std::vector<double> xyz, std::vector<std::string> atoms, std::vector<double> polaris, std::string id, size_t islocal,
                        int tag) {
    ...
    // Adding polarizability values 
    for (auto i = polaris.begin(); i != polaris.end(); i++) polaris_.push_back(*i);
    // Adding id
...
void System::AddMonomerInfo() {
    ...
    // Copy the polaris_ vector and clear it
    std::vector<double> polaris = polaris_;
    polaris_.clear();
    ...
    polaris_ = std::vector<double>(numsites_, 0.0);         //SY// check numat_ or numsites_
    ...
    for (size_t i = 0; i < monomers_.size(); i++) {
        ...
        // Polaris //SY//
        std::copy(polaris.begin() + fi_at[k], polaris.begin() + fi_at[k] + nat_[k], polaris_.begin() + count);
        // Atom names
